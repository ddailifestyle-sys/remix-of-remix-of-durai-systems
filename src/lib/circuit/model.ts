...
export type CircuitNode = {
  ...
  y: number;
  rotation: number;
  /** true when the part is mounted back/solder side up — pin order mirrors */
  flipped?: boolean;
...
export function createNode(def: PartDef, x: number, y: number): CircuitNode {
  return { id: newId(def.type), type: def.type, x, y, rotation: 0, flipped: false, props: defaultProps(def) };
}
