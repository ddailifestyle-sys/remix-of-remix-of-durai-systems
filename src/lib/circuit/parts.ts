...
/* ---------------------------------------------------------------------------
 * Real board pinouts. Pin ids stay in the D<n>/A<n> form the sketch parser and
 * validator use, while labels carry the silkscreen name of the physical pin.
 * ------------------------------------------------------------------------- */
...
const UNO_PINS: PartPin[] = [
  ...
  ...Array.from({ length: 14 }, (_, i) =>
    p(
      `D${i}`,
      i === 0 ? "D0 (RX)" : i === 1 ? "D1 (TX)" : i === 13 ? "D13 (LED)" : UNO_PWM.includes(i) ? `D${i} ~` : `D${i}`,
      UNO_PWM.includes(i) ? "pwm" : "digital",
      "top",
    ),
  ),
  ...Array.from({ length: 6 }, (_, i) => p(`A${i}`, `A${i}`, "analog", "bottom")),
  p("SDA", "SDA (A4)", "i2c", "top"),
  p("SCL", "SCL (A5)", "i2c", "top"),
];

const NANO_PINS: PartPin[] = [
  p("5V", "5V", "power", "bottom"),
  p("3V3", "3.3V", "power", "bottom"),
  p("VIN", "VIN", "power", "bottom"),
  p("GND", "GND", "ground", "bottom"),
  p("GND2", "GND", "ground", "bottom"),
  ...Array.from({ length: 14 }, (_, i) =>
    p(`D${i}`, UNO_PWM.includes(i) ? `D${i} ~` : `D${i}`, UNO_PWM.includes(i) ? "pwm" : "digital", "top"),
  ),
  ...Array.from({ length: 8 }, (_, i) => p(`A${i}`, `A${i}`, "analog", "bottom")),
  p("SDA", "SDA (A4)", "i2c", "top"),
  p("SCL", "SCL (A5)", "i2c", "top"),
];

/** Arduino Mega 2560: PWM on D2-D13, I2C on D20/D21, 16 analog inputs. */
const MEGA_PINS: PartPin[] = [
  p("5V", "5V", "power", "bottom"),
  p("3V3", "3.3V", "power", "bottom"),
  p("VIN", "VIN", "power", "bottom"),
  p("GND", "GND", "ground", "bottom"),
  ...
  ...Array.from({ length: 22 }, (_, i) =>
    p(`D${i}`, `D${i}${i >= 2 && i <= 13 ? " ~" : ""}`, i >= 2 && i <= 13 ? "pwm" : "digital", "top"),
  ),
  ...Array.from({ length: 16 }, (_, i) => p(`A${i}`, `A${i}`, "analog", "bottom")),
  p("SDA", "SDA (D20)", "i2c", "top"),
  p("SCL", "SCL (D21)", "i2c", "top"),
];

/**
 * ESP32 WROOM-32 DevKit v1: GPIO 6-11 are wired to the SPI flash and are not
 * broken out, GPIO 34-39 are input-only ADC1 channels, I2C defaults to 21/22.
 */
const ESP32_GPIO = [2, 4, 5, 12, 13, 14, 15, 16, 17, 18, 19, 23, 25, 26, 27, 32, 33];
const ESP32_ADC: [string, number][] = [
  ["A0", 36],
  ["A1", 39],
  ["A2", 34],
  ["A3", 35],
...
const ESP32_PINS: PartPin[] = [
  p("3V3", "3V3", "power", "bottom"),
  p("5V", "5V (VIN)", "power", "bottom"),
  p("GND", "GND", "ground", "bottom"),
  p("GND2", "GND", "ground", "bottom"),
  ...ESP32_GPIO.map((g) => p(`D${g}`, `GPIO${g}`, "pwm", "top")),
  ...ESP32_ADC.map(([id, g]) => p(id, `GPIO${g} / ${id}`, "analog", "bottom")),
  p("SDA", "SDA (GPIO21)", "i2c", "top"),
  p("SCL", "SCL (GPIO22)", "i2c", "top"),
];

/** ESP32-S3 camera board: most GPIO are consumed by the OV2640 bus. */
const S3_CAM_GPIO = [1, 2, 3, 14, 19, 20, 21, 45, 46, 47, 48];
const S3_CAM_PINS: PartPin[] = [
  p("3V3", "3V3", "power", "bottom"),
  p("5V", "5V", "power", "bottom"),
  p("GND", "GND", "ground", "bottom"),
  p("GND2", "GND", "ground", "bottom"),
  ...S3_CAM_GPIO.map((g) => p(`D${g}`, `GPIO${g}`, "pwm", "top")),
  ...([
    ["A0", 1],
    ["A1", 2],
    ["A2", 3],
  ] as [string, number][]).map(([id, g]) => p(id, `GPIO${g} / ${id}`, "analog", "bottom")),
  p("SDA", "SDA (GPIO21)", "i2c", "top"),
  p("SCL", "SCL (GPIO47)", "i2c", "top"),
];

/** NodeMCU ESP8266: silkscreen D0-D8 map to fixed GPIO, single ADC pin A0. */
const NODEMCU_MAP: [number, number][] = [
  [0, 16],
  [1, 5],
  [2, 4],
  [3, 0],
...
const ESP8266_PINS: PartPin[] = [
  p("3V3", "3V3", "power", "bottom"),
  p("5V", "VU (5V)", "power", "bottom"),
  p("GND", "GND", "ground", "bottom"),
  p("GND2", "GND", "ground", "bottom"),
  ...NODEMCU_MAP.map(([d, g]) => p(`D${d}`, `D${d} (GPIO${g})`, "pwm", "top")),
  p("A0", "A0 (ADC0)", "analog", "bottom"),
  p("SDA", "SDA (D2)", "i2c", "top"),
  p("SCL", "SCL (D1)", "i2c", "top"),
];

/** Raspberry Pi Pico W: GP0-GP22 digital, GP26-GP28 are the three ADC inputs. */
const PICO_PINS: PartPin[] = [
  p("3V3", "3V3 OUT", "power", "bottom"),
  p("5V", "VSYS (5V)", "power", "bottom"),
  p("GND", "GND", "ground", "bottom"),
  p("GND2", "GND", "ground", "bottom"),
  ...Array.from({ length: 23 }, (_, i) => p(`D${i}`, `GP${i}`, "pwm", "top")),
  ...([
    ["A0", 26],
    ["A1", 27],
    ["A2", 28],
  ] as [string, number][]).map(([id, g]) => p(id, `GP${g} / ${id}`, "analog", "bottom")),
  p("SDA", "SDA (GP4)", "i2c", "top"),
  p("SCL", "SCL (GP5)", "i2c", "top"),
];
...
export const PARTS: PartDef[] = [
  ...
  },
  {
    ...
    description: "Light dependent resistor module — analog level plus a threshold digital output.",
  },
  ...
  },
  {
    ...
    description: "Microphone module — analog envelope plus a comparator digital output.",
  },
  ...
  },
  {
    ...
    description: "NDIR carbon dioxide sensor — talks UART (TX/RX) to the board.",
  },
  ...
  },
  {
    ...
    pins: [
      ...
      p("SDA", "SDA/SS", "digital", "top", true),
      p("SCK", "SCK", "digital", "top", true),
      p("MOSI", "MOSI", "digital", "top", true),
      p("MISO", "MISO", "digital", "top", true),
      p("RST", "RST", "digital", "top"),
      p("GND", "GND", "ground", "bottom", true),
    ],
    description: "13.56 MHz RFID reader on the SPI bus (SDA/SCK/MOSI/MISO/RST).",
  },
