...
export interface Hardware {
  ...
  /** ESP32 camera peripheral: init / capture / release. Returns frame size in bytes. */
  camera(op: string, args: Value[]): number;
}
...
class Parser {
  ...
  tryFunction(): FuncDecl | null {
    ...
    while (!this.is(")") && this.peek().t !== "eof") {
      this.skipTypeTokens();
      // not a parameter list (e.g. `LiquidCrystal_I2C lcd(0x27, 16, 2);`) — this is
      // a class instance declaration, not a function definition
      if (this.peek().t !== "id") { this.i = start; return null; }
...
export class Sketch {
  ...
  /** `consts` overrides board-specific constants (e.g. A0 = 36 on an ESP32). */
  constructor(private program: Program, private hw: Hardware, private consts: Record<string, Value> = {}) {}

  get objects() { return this.program.objects; }

  init() {
    for (const [k, v] of Object.entries({ ...CONSTS, ...this.consts })) this.globals.declare(k, v);
  ...
  private call(e: Expr & { k: "call" }, scope: Scope): Value {
    ...
    switch (name) {
      ...
      case "esp_camera_init":
      case "esp_camera_capture":
      case "esp_camera_fb_get":
      case "esp_camera_fb_return":
      case "cameraCapture":
        return this.hw.camera(name, args);
      case "map": {
