...
export class SketchRunner {
  ...
  private hardware(): Hardware {
    ...
    return {
      ...
      camera(op, _args: Value[]) {
        if (op === "esp_camera_init") {
          self.log("info", "camera initialised");
          return 0;
        }
        if (op === "esp_camera_capture") {
          self.log("info", "camera frame captured");
          return 1024;
        }
        return 0;
      },
...
export function buildSuite(problem: Problem): TestCase[] {
  ...
  for (let i = 0; i < total; i++) {
    ...
    // Bands are normalised to equal 0-100 % slices in declaration order, so this
    // suite and the generated sketch always agree regardless of the units the
    // logic table uses (%, ppm, AQI, seconds).
