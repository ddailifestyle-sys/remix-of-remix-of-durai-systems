...
import { Cable, CircuitBoard, Code2, Play, Search, Square, StepForward, Trash2 } from "lucide-react";
...
import { CircuitIO } from "@/components/circuit/CircuitIO";
import { AutosaveBadge, type SaveStatus } from "@/components/circuit/AutosaveBadge";
import type { CircuitFile } from "@/components/circuit/CircuitIO";
...
import { boardPinUsage } from "@/lib/circuit/board-map";
...
function Builder() {
  ...
  }, [circuit, code, scenario, view, problemId]);

  useEffect(() => {
    if (!hydrated) return;
    setSaveStatus("saving");
    const id = window.setTimeout(writeNow, 700);
  ...
  const start = (sketch?: string) => {
    const engine = new SimulationEngine(circuit, sketch ?? code, scenario);
  ...
  /** generate the sketch from the circuit + scenario, then run it immediately */
  const generateAndRun = useCallback(() => {
    const gen = generateSketch(circuit, scenario);
    if (!gen.code) {
      toast.error(gen.notes[0] ?? "Place a board and a few components first.");
      return;
    }
    setCode(gen.code);
    stop();
    window.setTimeout(() => start(gen.code), 0);
    toast.success(`Running generated sketch for ${gen.boardLabel}`);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [circuit, scenario, code]);
  ...
  const usage = useMemo(() => boardPinUsage(circuit), [circuit]);

  const highlightedPins = useMemo(() => (activeIssue !== null ? issues[activeIssue]?.pins ?? [] : []), [issues, activeIssue]);
  const highlightedWires = useMemo(() => (activeIssue !== null ? issues[activeIssue]?.wireIds ?? [] : []), [issues, activeIssue]);

  const importCircuit = useCallback((file: CircuitFile) => {
    setCircuit(file.circuit);
    setCode(file.code);
    setScenario({ ...defaultScenario(), ...file.scenario });
    ...
    <div>
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <AutosaveBadge status={saveStatus} savedAt={savedAt} />

          {running ? (
            <button onClick={stop} aria-label="Stop simulation" className="inline-flex items-center gap-2 rounded-xl border border-danger px-5 py-3 text-sm font-semibold text-danger focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan">
              <Square className="h-4 w-4" aria-hidden="true" /> Stop
            </button>
          ) : (
            <button onClick={() => start()} aria-label="Run simulation" className="inline-flex items-center gap-2 rounded-xl bg-brand-gradient px-5 py-3 text-sm font-semibold text-primary-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan">
              <Play className="h-4 w-4" aria-hidden="true" /> Run Simulation
            </button>
          )}
          <button
            onClick={() => {
            ...
            aria-label="Step simulation once"
            className="inline-flex items-center gap-2 rounded-xl border border-border px-4 py-3 text-sm font-semibold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan"
          >
            <StepForward className="h-4 w-4" aria-hidden="true" /> Step
          </button>
          <button onClick={resetWorkspace} aria-label="Reset workspace" className="inline-flex items-center gap-2 rounded-xl border border-border px-4 py-3 text-sm font-semibold text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan">
            <Trash2 className="h-4 w-4" aria-hidden="true" /> Reset
          </button>
        </div>
      </div>
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-[240px_1fr_320px]">
        <aside className="panel max-h-[720px] overflow-auto p-4">
          ...
          {usage && (
            <section aria-labelledby="pin-usage-heading" className="mt-3 rounded-lg border border-border bg-surface-2 p-2">
              <h3 id="pin-usage-heading" className="text-[11px] font-semibold uppercase text-cyan">
                {usage.boardLabel} pins
              </h3>
              <p className="mt-0.5 text-[10px] text-muted-foreground">
                {usage.usedCount} used · {usage.freeCount} free — click a used pin to jump to its wire.
              </p>
              <ul className="mt-2 flex flex-wrap gap-1">
                {usage.pins.map((p) => {
                  const used = p.users.length > 0;
                  const first = p.users[0];
                  return (
                    <li key={p.pinId}>
                      <button
                        type="button"
                        disabled={!used}
                        onClick={() => first && setSelected({ kind: "wire", id: first.wireId })}
                        title={
                          used
                            ? `${p.label} → ${p.users.map((u) => `${u.component} ${u.componentPin}`).join(", ")}`
                            : `${p.label} is free (${p.kind})`
                        }
                        aria-label={
                          used
                            ? `${p.label} in use by ${p.users.map((u) => u.component).join(", ")}`
                            : `${p.label} free`
                        }
                        className={`rounded border px-1.5 py-0.5 font-mono text-[10px] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan ${
                          used
          ...
          <div className="sticky top-0 z-10 -mx-4 mt-3 bg-card px-4 pb-3">
            <div className="relative">
              ...
              <input
                ...
                className="w-full rounded-lg border border-input bg-background py-2 pl-8 pr-2 text-xs outline-none focus:border-cyan focus-visible:ring-2 focus-visible:ring-cyan"
              />
            </div>
            <div className="mt-2 flex flex-wrap gap-1">
              {["All", ...categories.map(([c]) => c)].map((c) => (
                <button
                  ...
                  className={`rounded-full border px-2 py-0.5 text-[10px] font-semibold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan ${
        ...
        <div className="space-y-6">
          <div className="panel p-4">
            ...
            <CircuitCanvas
              ...
              outputs={outputs}
              running={running}
              highlightPins={highlightedPins}
              highlightWires={highlightedWires}
            />
          </div>
