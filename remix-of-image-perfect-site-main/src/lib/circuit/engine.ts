...
export class SimulationEngine {
  ...
  step(): StepResult {
    ...
    return {
      ...
      pins: Object.fromEntries([...this.pinValues].map(([k, v]) => [pinName(k), v])),
  ...
  private noisy(v: number) {
    const n = this.scenario.noise;
    return n <= 0 ? v : v + (Math.random() * 2 - 1) * n;
  }

  private channelValue(ch: SensorChannel) {
    return this.noisy(this.scenario.channels[ch] ?? 0);
  }
  ...
  private hardware(): Hardware {
    ...
    return {
      ...
      digitalWrite(pin, value) {
        ...
        if (prev !== (value ? 1 : 0))
          self.log("io", `digitalWrite(${pinName(pin)}, ${value ? "HIGH" : "LOW"})${sink ? ` → ${nodeDef(sink.node)?.label}` : ""}`);
      },
      digitalRead(pin) {
      ...
      analogRead(pin) {
        const src = self.sourceOn(pin);
        if (!src) return 0;
        return self.analogOf(src.node);
      },
      analogWrite(pin, value) {
        self.pinValues.set(pin, clamp(value, 0, 255));
        const sink = self.sinkOn(pin);
        self.log("io", `analogWrite(${pinName(pin)}, ${Math.round(value)})${sink ? ` → ${nodeDef(sink.node)?.label}` : ""}`);
      },
      delay(ms) {
        self.clock += ms;
      },
      ...
      tone(pin, freq) {
        ...
        self.log("io", `tone(${pinName(pin)}, ${Math.round(freq)} Hz)`);
      },
      noTone(pin) {
